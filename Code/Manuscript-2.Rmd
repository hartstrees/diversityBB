...
### Import libraries
ipak(c('tidyverse', 'tidymodels', 'reshape2', 'here', 'rgdal', 'sp', 'raster', 'stars', 'sf', 'prism', 'tigris', "tmap", 'fasterize', 'ggplot2','RColorBrewer','ggsci', 'patchwork', 'colorspace', 'grid', 'knitr',  'flextable', 'bookdown',  'pdp', 'ranger', 'ALEPlot', 'VSURF', 'themis', 'foreach',  'FSA', 'parallel'))
...
In the western United States, native bark beetles (Curculionidae: Scolytinae) have killed 3.8 billion trees over the past two decades [1997-2018; @hicke_characterizing_2020]. While across the region there are more than 25 native bark beetle species, the mountain pine beetle (MPB; *Dendroctonus ponderosae*), spruce beetle (SB;  *Dendroctonus rufipennis*), and western balsam bark beetle (WBBB; *Dryocoetes confusus*) collectively have caused over 70% of the bark beetle-attributed tree mortality, most of which has occurred in high elevation forests [@hicke_chacterizing_2020].

In the case of plant-herbivore interactions, greater plant diversity often reduces the effects of herbivory [i.e., 'associational resistance', @barbosa_associational_2009], particularly when herbivores are specialists capable of rapid population growth [@otway_resource_2005]. Associational resistance is commonly hypothesized to occur due to several non-mutually exclusive mechanisms. First, herbivory may be lower in more diverse systems if herbivores are less likely to encounter, stay, and reproduce in areas where their hosts are less abundant [i.e., 'resource concentration hypothesis', @root_organization_1973]. Second, reductions in herbivory may occur if insects are less likely to find their hosts due to chemical or visual masking by heterospecific neighbors [i.e. 'host apparency hypothesis', @feeny_plant_1976, @sciebe_semiochemical_2011, @castagneryrol_effects_2014], Third, heterospecific neighbors may increase the abundance of herbivore natural enemies [i.e., 'enemies hypothesis', @elton_ecology_1985, @staab_influence_2020]. However, evidence for associational resistance in natural (i.e. not plantations) forested ecosystems is limited.

While increases in plant diversity may decrease the effects of herbivore species on individuals and populations of plants, it is also hypothesized to increase herbivore richness ['resource specialization hypothesis', @keddy_plant_1984], which may result in more efficient resource use and greater depletion of the plant community, especially when herbivores are specialists [@cardinale_effects_2005; @finke_niche_2008]. 

Yet little research has examined the effects of plant diversity on community-level outcomes of herbivory, which critically may drive key ecosystem processes. This is particularly true in natural systems, where the number of potential interactions is high, the effects of different herbivores on their focal host populations often differ greatly, and the changes in plant diversity are often convoluted with changes in host quality [@jactel_tree_2021]. 
...
To better understand interactions between tree and bark beetle communities in subalpine forests, we used a large dataset consisting of XXX,XXX plots established by the United States Forest Service Forest Inventory and Analysis Program (FIA; https://www.fia.fs.fed.us/). Specifically, we ask:

(1) How often do stand conditions suitable for multiple bark beetle species occur?
(2) Does host species richness effect the occurrence or severity of cumulative bark beetle activity increase with the host species richness?
...
When stand conditions are suitable for multiple bark beetle species, we expect that the 
...
df$agentSBo <- ifelse(df$TPH.PIEN>0, ifelse(df$stand.SB/(df$TPH.PIEN/6.018046/2.47105)>0.1, "SB", "absent"), "absent")
df$agentMPBo <- ifelse(df$TPH.PICO>0, ifelse(df$stand.MPB/(df$TPH.PICO/6.018046/2.47105)>0.1, "MPB", "absent"), "absent")
df$agentWBBBo <- ifelse(df$TPH.ABLA>0, ifelse(df$stand.WBBB/(df$TPH.ABLA/6.018046/2.47105)>0.1, "WBBB", "absent"), "absent")
...
df$agentso <-  do.call(paste, c(df[c("agentSBo", "agentMPBo", "agentWBBBo")],sep="-"))
df$agentso <- factor(df$agentso, levels=c("absent-absent-absent","absent-MPB-absent","SB-absent-absent", "SB-MPB-absent", "absent-absent-WBBB", "SB-absent-WBBB", "absent-MPB-WBBB", "SB-MPB-WBBB"), labels=c("none", "MPB", "SB", "MPB+SB", "WBBB", "SB+WBBB", "MPB+WBBB", "MPB+SB+WBBB"))

df$nagentso <- df$agentso
df$nagentso <- factor(df$nagentso, levels=c("none", "MPB", "SB", "MPB+SB", "WBBB", "SB+WBBB", "MPB+WBBB", "MPB+SB+WBBB"), labels=c(0, 1, 1, 2, 1, 2, 2, 3))
...
df$hostsPICO25 <- ifelse(df$BA.PICO/df$BA.total>0.25, "PICO", "absent")
df$hostsPIEN25 <- ifelse(df$BA.PIEN/df$BA.total>0.25, "PIEN", "absent")
df$hostsABLA25 <- ifelse(df$BA.ABLA/df$BA.total>0.25, "ABLA", "absent")
df$hosts25 <-  do.call(paste, c(df[c("hostsPICO25", "hostsPIEN25", "hostsABLA25")],sep="-"))
df$hosts25 <- factor(df$hosts25, levels=c('PICO-absent-absent','absent-absent-ABLA', 'absent-PIEN-absent', 'PICO-PIEN-absent',   'PICO-absent-ABLA', 'absent-PIEN-ABLA', 'PICO-PIEN-ABLA', 'absent-absent-absent'), labels= c('PICO', 'ABLA', 'PIEN', 'PICO+PIEN', 'PICO+ABLA', 'PIEN+ABLA', 'PICO+PIEN+ABLA', 'none'), ordered=TRUE)

df$nhosts25 <- df$hosts25
df$nhosts25 <- factor(df$nhosts25, levels=c('PICO', 'ABLA', 'PIEN', 'PICO+PIEN', 'PICO+ABLA', 'PIEN+ABLA', 'PICO+PIEN+ABLA', 'none'), labels=c(1, 1,1,2,2,2,3,0))
...
df <- read.csv(here("Data/Processed/FIAdat-recent.csv"))
...
### Affected
df$affected <- ifelse(df$stand.BB>0, "affected", "unaffected")

### Affected
df$affectedo <- ifelse(df$stand.BB/df$BA.Allhosts>0.1, "affected", "unaffected")


write.csv(df, here("Data/Processed/FIAdat-recent-re.csv"),row.names=F)
```

```{r import processed data}
df <- read.csv( here("Data/Processed/FIAdat-recent-re.csv"))
...
df$hostsF <- factor(df$hosts, levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO & PIEN", "PICO & ABLA", "PIEN & ABLA", "PICO, PIEN, & ABLA"), ordered=T)
df$hostsFP <- factor(df$hosts,levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"))

df$nhosts25F <-factor(df$nhosts25, levels=1:3, labels=c("one", "two", "three"), ordered=T)
df$hosts25 <- factor(df$hosts25, levels=c("none", "PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), ordered=T)
df$hosts25F <- factor(df$hosts25, levels=c("none","PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("none", "PICO", "PIEN", "ABLA", "PICO & PIEN", "PICO & ABLA", "PIEN & ABLA", "PICO, PIEN, & ABLA"), ordered=T)
df$hostsF25P <- factor(df$hosts25,levels=c("none", "PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("none", "PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"))
...
df$nagentsoF <- factor(df$nagentso, levels=0:3, ordered=T)
df$agentsoF <- factor(df$agentso, levels=c("none", "MPB", "SB", "WBBB", "MPB+SB","MPB+WBBB",  "SB+WBBB", "MPB+SB+WBBB"), ordered=T)
...
df.all$agentsFlong <-factor(df.all$agentsF, levels=levels(df.all$agentsF), labels=c("unaffected", "MPB", "SB", "WBBB", "MPB & SB", "MPB & WBBB", "SB & WBBB", "MPB, SB, & WBBB"))
...
map1 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PICO",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=c(unaffected="#A6A6A6", MPB="#710002")) +tm_compass(position=c(0.1, 0.02), just=c("center", "bottom"), text.size=0.5, size=2)+tm_layout(main.title = "PICO",main.title.size=0.5,main.title.position="center")

map2 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PIEN",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col)) +tm_layout(main.title = "PIEN",main.title.size=0.5,main.title.position="center")

map3 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="ABLA",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col)) +tm_layout(main.title = "ABLA",main.title.size=0.5, main.title.position="center")

map4 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PICO+PIEN",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col)) +tm_layout(main.title = "PICO & PIEN",main.title.size=0.5,main.title.position="center")

map5 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PICO+ABLA",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col)) +tm_layout(main.title = "PICO & ABLA", main.title.size=0.5, main.title.position="center")

map6 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PIEN+ABLA",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col)) +tm_layout(main.title = "PIEN & ABLA", main.title.size=0.5, main.title.position="center")

map7 <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PICO+PIEN+ABLA",]) +tm_dots(col="agentsFlong", legend.show = FALSE,size=.0025, palette=rev(p2col))+tm_layout(main.title = "PICO, PIEN, & ABLA", main.title.size=0.5, main.title.position="center")

legend.map <- tm_shape(IMW, bbox=tmaptools::bb(ext)) +  tm_fill(col=grey(level=0.95)) +  tm_borders()  + tm_shape(df.all.sf[df.all.sf$hosts=="PICO+PIEN+ABLA",]) +tm_dots(col="agentsFlong", size=4, title="bark beetle species",  palette=rev(p2col))+tm_layout(legend.only = TRUE,legend.text.size = 0.5,legend.title.size = 0.65)+tm_scale_bar(position = c(0.5,0.16), text.size=0.5, just="center", breaks=c(0,250,500))

Fig.file <- here("Results/Figures/Figure-Map-Anyhost-Anybeetle.jpg")
...
```{r FigMap, fig.cap="The distribution of FIA plots affected by outbreaks of the mountain pine beetle (MPB), spruce beetle (SB), and western balsam bark beetle (WBBB) by the identity of host tree species present within each plot. For host identities, PICO is lodgepole pine, PIEN is Engelmann spruce, and ABLA is subalpine fir.", out.width = "504.8pt"}
knitr::include_graphics(here("Results/Figures/Figure-Map-Anyhost-Anybeetle.jpg"))
```
...
## Do stand conditions conducive for outbreaks of multiple bark beetle species commonly co-occur?
...
```{r set up RF helper model}
rf.mod <- function(response="agentMPB", vars=c("BA.PICO", "QMD.PICO", "propBA.PICO"), datBB.c=df[df$focalhost=="Any",]){
  ...
  colnames(x) <- c("PLT_CN", paste0("pred", response))
  ...
  pdale <- NULL
  for(z in vars.final){
    pdz <- partial(ranger_obj, train=datz.train, pred.var=z,   which.class=strsplit(strsplit(response, split="agent")[[1]][2], split="o")[[1]][1], prob=T,   trim.outliers=T)
    pdz$var <- z
    colnames(pdz) <- c("value","yhat","var")
    pdz$method <- "PD"
    
    alez <- ALEPlot(X=datz.train[,vars.final], X.model=ranger_obj, pred.fun=function(X.model,   newdata){predict(X.model, newdata)$predictions[,2]}, J=which(vars.final==z), K=50)
    alez <- data.frame(value=alez$x.values, yhat=alez$f.values, var=z, method="ALE")
    
    pdale <- rbind(pdale, pdz)
    pdale <- rbind(pdale, alez)
  }
  write.csv(pdale,  here(paste0("Results/RF-partialdependence-", response, "-occurrence-SMOTE.csv")), row.names=F)
}
```

```{r fit MPB RF model occurrence, eval=F, fig.show='hide'}
rf.mod(response="agentMPB", vars=c("BA.PICO", "QMD.PICO", "propBA.PICO") , datBB.c =df[df$focalhost=="Any",])
```

```{r fit SB RF model occurrence, eval=F, fig.show='hide'}
rf.mod(response="agentSB", vars=c("BA.PIEN", "QMD.PIEN", "propBA.PIEN") , datBB.c =df[df$focalhost=="Any",])
```

```{r fit WBBB RF model occurrence, eval=F, fig.show='hide'}
rf.mod(response="agentWBBB", vars=c("BA.ABLA", "QMD.ABLA", "propBA.ABLA") , datBB.c =df[df$focalhost=="Any",])
```

```{r fit MPB RF model outbreak, eval=F, fig.show='hide'}
rf.mod(response="agentMPBo", vars=c("BA.PICO", "QMD.PICO", "propBA.PICO") , datBB.c =df[df$focalhost=="Any",])
```

```{r fit SB RF model outbreak, eval=F, fig.show='hide'}
rf.mod(response="agentSBo", vars=c("BA.PIEN", "QMD.PIEN", "propBA.PIEN") , datBB.c =df[df$focalhost=="Any",])
```

```{r fit WBBB RF model outbreak, eval=F, fig.show='hide'}
rf.mod(response="agentWBBBo", vars=c("BA.ABLA", "QMD.ABLA", "propBA.ABLA") , datBB.c =df[df$focalhost=="Any",])
```
...
MPBopred <- read.csv(here("Results/RF-pred-agentMPBo-occurrence-SMOTE.csv"))
SBopred <- read.csv(here("Results/RF-pred-agentSBo-occurrence-SMOTE.csv"))
WBBBopred <- read.csv(here("Results/RF-pred-agentWBBBo-occurrence-SMOTE.csv"))


BBpred <- merge(merge(MPBpred, SBpred, by="PLT_CN"), WBBBpred, by="PLT_CN")
colnames(BBpred) <- c("PLT_CN", "MPBpred", "SBpred", "WBBBpred")

BBopred <- merge(merge(MPBopred, SBopred, by="PLT_CN"), WBBBopred, by="PLT_CN")
colnames(BBopred) <- c("PLT_CN", "MPBopred", "SBopred", "WBBBopred")

BBpred <- merge(BBpred, BBopred, by="PLT_CN")
...
BBpred$MPBosuitable <- ifelse(BBpred$MPBopred > threshold, "MPB", "absent")
BBpred$SBosuitable <- ifelse(BBpred$SBopred > threshold, "SB", "absent")
BBpred$WBBBosuitable <- ifelse(BBpred$WBBBopred >threshold, "WBBB", "absent")
...
BBpred$agentsuitableo <-  do.call(paste, c(BBpred[c("SBosuitable", "MPBosuitable", "WBBBosuitable")],sep="-"))
BBpred$agentsuitableo <- factor(BBpred$agentsuitableo, levels=c("absent-absent-absent","absent-MPB-absent","SB-absent-absent", "SB-MPB-absent", "absent-absent-WBBB", "SB-absent-WBBB", "absent-MPB-WBBB", "SB-MPB-WBBB"), labels=c("none", "MPB", "SB", "MPB+SB", "WBBB", "SB+WBBB", "MPB+WBBB", "MPB+SB+WBBB"))

BBpred$suitablehostso <- factor(BBpred$agentsuitableo, levels=c("none", "MPB", "SB", "WBBB", "MPB+SB",  "MPB+WBBB", "SB+WBBB",  "MPB+SB+WBBB"), ordered=T)
```

```{r determine co-occurrence hosts, eval=F}
dfpred<- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred$suitablehosts <- factor(dfpred$suitablehosts, levels=levels(dfpred$suitablehosts), labels=c('none', levels(dfpred$hosts)))
dfpred$suitablehostso <- factor(dfpred$suitablehostso, levels=levels(dfpred$suitablehostso), labels=c('none', levels(dfpred$hosts)))

hostgroups <- dfpred[dfpred$focalagent=="Any",]%>% group_by(hosts) %>% summarise(n = n())%>%mutate(freq = n / sum(n))
colnames(hostgroups) <- c("hosts", "n", "freq")
hostgroups$var <- "all"

suitablehostgroups <- dfpred[dfpred$focalagent=="Any",]%>% group_by(suitablehosts) %>% summarise(n = n())%>%mutate(freq = n / sum(n))
colnames(suitablehostgroups) <- c("hosts", "n", "freq")
suitablehostgroups$var <- "suitable"

suitablehostgroupso <- dfpred[dfpred$focalagent=="Any",]%>% group_by(suitablehostso) %>% summarise(n = n())%>%mutate(freq = n / sum(n))
colnames(suitablehostgroupso) <- c("hosts", "n", "freq")
suitablehostgroupso$var <- "suitableo"

hostz <- rbind(hostgroups, suitablehostgroups)
hostz <- rbind(hostz, suitablehostgroupso)
...
To determine if the proportion of plots affected by any bark beetle species varied with the number or identity of hosts present, we first used a proportion test to determine if groups differed followed by a pairwise proportion test to test for differences between groups. To account for multiple comparisons, we adjusted p-values to account for multiple comparisons [@benjamini_false_1995].
...
```{r BB occurrence by hosts, eval=T}
...
resp$Letter2 <- c("a", "b", "bc", "c", "cd", "d", "e")
...
ggpallpres <- ggplot(resp.hosts, aes(x=hosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Presence-agentpresence-hostpresence.jpg"), plot=ggpallpres, width=fig.width1,height=fig.width1,units="in")
```

```{r BB outbreak occurrence by hosts, eval=T}
res <- dfpred %>% count(hosts, affectedo) %>% group_by(hosts) %>% mutate(prop = prop.table(n))
x <- dfpred %>% count(hosts) 

resa <- res[res$affectedo=="affected",]
resa$alln <- x$n
...
resp <- res[res$affectedo=="affected",]
resp <- merge(resp, xxpr, by.x="hosts", by.y="Group")
resp <- resp[order(resp$prop),]
resp$Letter2 <- c("a", "b", "b", "c", "cd", "d", "d")

resp$hosts <- factor(resp$hosts, levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)
resp.hosts <- resp

ggpallout <- ggplot(resp.hosts, aes(x=hosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")


ggsave(here("Results/Figures/Figure-Presence-agentoutbreak-hostspresence.jpg"), plot=ggpallout, width=fig.width1,height=fig.width1,units="in")
```

```{r BB occurrence by suitablehosts}
## Suitable hosts
res <- dfpred[!dfpred$suitablehosts=="none",] %>% count(suitablehosts, affected) %>% group_by(suitablehosts) %>% mutate(prop = prop.table(n))
x <- dfpred[!dfpred$suitablehosts=="none",] %>% count(suitablehosts) 
...
resp <- resp[order(resp$prop),]
resp$Letter2 <- c("a", "b", "c", "c", "dc", "de", "e")

resp$suitablehosts <- factor(as.character(resp$suitablehosts), levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

resp.suitablehosts <- resp

ggpsuitablepres <- ggplot(resp.suitablehosts, aes(x=suitablehosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Presence-agentpresence-suitablehosts.jpg"), plot=ggpsuitablepres, width=fig.width1,height=fig.width1,units="in")

```

```{r BB outbreak occurrence by suitable hosts}
## Suitable hosts
res <- dfpred[!dfpred$suitablehosts=="none",] %>% count(suitablehosts, affectedo) %>% group_by(suitablehosts) %>% mutate(prop = prop.table(n))
x <- dfpred[!dfpred$suitablehosts=="none",] %>% count(suitablehosts) 
...
resp <- merge(resp, xxpr, by.x="suitablehosts", by.y="Group")
resp <- resp[order(resp$prop),]
resp$Letter2 <- c("a", "b", "b", "c", "c", "c", "c")

resp$suitablehosts <- factor(as.character(resp$suitablehosts), levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

resp.suitablehostso <- resp

ggpsuitableoutbreak<- ggplot(resp.suitablehostso, aes(x=suitablehosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")


ggsave(here("Results/Figures/Figure-Presence-agentoutbreak-suitablehosts.jpg"), plot=ggpsuitableoutbreak, width=fig.width1,height=fig.width1,units="in")
```

```{r BB occurrence by hosts25}
## 
res <- df[!df$hosts25=="none",] %>% count(hosts25, affected) %>% group_by(hosts25) %>% mutate(prop = prop.table(n))
x <-  df[!df$hosts25=="none",] %>% count(hosts25) 
...
resp <- res[res$affected=="affected",]
resp <- merge(resp, xxpr, by.x="hosts25", by.y="Group")
resp <- resp[order(resp$prop),]
resp$Letter2 <- c("a", "b", "b", "c", "c", "c", "bc")

resp$hosts25 <- factor(as.character(resp$hosts25), levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

resp.hosts25 <- resp

ggphost25pres <- ggplot(resp.hosts25, aes(x=hosts25,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")


ggsave(here("Results/Figures/Figure-Presence-agentpresence-hosts25.jpg"), plot=ggphost25pres, width=fig.width1,height=fig.width1,units="in")
```

```{r BB outbreak occurrence by hosts25}
## Hosts 25
res <- df[!df$hosts25=="none",] %>% count(hosts25, affectedo) %>% group_by(hosts25) %>% mutate(prop = prop.table(n))
x <- df[!df$hosts25=="none",] %>% count(hosts25) 
...
resp <- merge(resp, xxpr, by.x="hosts25", by.y="Group")
resp <- resp[order(resp$prop),]
resp$Letter2 <- c("a", "abc", "b", "b", "c", "c", "c")

resp$hosts25 <- factor(as.character(resp$hosts25), levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

resp.hosts25o <- resp

ggphost25outbreak <- ggplot(resp.hosts25o, aes(x=hosts25,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Presence-agentoutbreak-hosts25.jpg"), plot=ggphost25outbreak, width=fig.width1,height=fig.width1,units="in")
```

```{r BB severity by hosts, eval=F}
dfpred <- df
dfpred <- dfpred[dfpred$focalagent=="Any",]
...
res2 <- dfpred %>% group_by(hosts) %>% summarize(median=median(propTBA), max=max(propTBA), n=n())

res3 <- merge(res, res2, by.x="Group", by.y="hosts")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "ab", "bc", "c", "cd", "d", "d")
res3$Group <- factor(res3$Group, levels=levels(df$hosts), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

ggpallseverity <- ggplot(dfpred, aes(x=hostsFP, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group)) + geom_text(data=res3, aes(label = n, x = Group, y = median*100 + 4, fill=Group),size = 3) + ylab("severity (% BA)") +xlab("hosts")+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Severity-agentpresence-hostpresence.jpg"), plot=ggpallseverity, width=fig.width1,height=fig.width1,units="in")
```

```{r BB outbreak severity by hosts, eval=F}
dfpred <- df
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[dfpred$affectedo=="affected",]
...
res3 <- merge(res, res2, by.x="Group", by.y="hosts")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "b", "bc", "bc", "cd", "cd", "d")
res3$Group <- factor(res3$Group, levels=levels(df$hosts), labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

ggoallseverity <- ggplot(dfpred, aes(x=hostsFP, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey") + geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group)) + geom_text(data=res3, aes(label = n, x = Group, y = median*100 +4, fill=Group),size = 3) + ylab("severity (% BA)") +xlab("hosts") + theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Severity-agentoutbreak-hostpresence.jpg"), plot=ggoallseverity, width=fig.width1,height=fig.width1,units="in")
```

```{r BB severity by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[dfpred$affected=="affected",]
...
res2 <- dfpred %>% group_by(suitablehosts) %>% summarize(median=median(propTBA), max=max(propTBA),n=n())

res3 <- merge(res, res2, by.x="Group", by.y="suitablehosts")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "b", "b", "bc", "b", "c", "bc")

ggpsuitablehostsseverity <- ggplot(dfpred, aes(x=suitablehosts, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ geom_text(data=res3, aes(label = n, x = Group, y = median*100 +2, fill=Group),size = 3)+ ylab("severity (% BA)") +xlab("suitable hosts")+theme(legend.position="none")


ggsave(here("Results/Figures/Figure-Severity-agentpresence-suitbalehosts.jpg"), plot=ggpsuitablehostsseverity, width=fig.width1,height=fig.width1,units="in")
```

```{r BB outbreak severity by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[dfpred$affectedo=="affected",]
dfpred <- dfpred[!dfpred$suitablehosts=="none",]
...
res2 <- dfpred %>% group_by(suitablehosts) %>% summarize(median=median(propTBA), max=max(propTBA), n=n())
res2$n2 <- paste0("n = ", res2$n)
res3 <- merge(res, res2, by.x="Group", by.y="suitablehosts")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "b", "bc", "bc", "cd", "cd", "d")

ggosuitablehostsseverity <- ggplot(dfpred, aes(x=suitablehosts, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ geom_text(data=res3, aes(label = n, x = Group, y = median*100 +4, fill=Group),size = 3)+ylab("severity (% BA)") +xlab("suitable hosts")+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Severity-agentoutbreak-suitbalehosts.jpg"), plot=ggosuitablehostsseverity, width=fig.width1,height=fig.width1,units="in")
```

```{r BB severity by hosts25, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[dfpred$affected=="affected",]
dfpred <- dfpred[!dfpred$hosts25=="none",]
dfpred$hosts25F <- factor(dfpred$hosts25, levels=levels(dfpred$hosts25)[-1],  labels=c("PICO", "PIEN", "ABLA", "PICO\n&\nPIEN", "PICO\n&\nABLA", "PIEN\n&\nABLA", "PICO,\nPIEN,\n&\nABLA"), ordered=T)

x <- kruskal.test(propTBA~hosts25F, data=dfpred)
DT <- dunnTest(propTBA~hosts25F, data=dfpred,method="bh")
res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.05)

res2 <- dfpred %>% group_by(hosts25F) %>% summarize(median=median(propTBA), max=max(propTBA), n=n())
res2$n2 <- paste0("n = ", res2$n)
res3 <- merge(res, res2, by.x="Group", by.y="hosts25F")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("abc", "a", "b", "b", "bc", "c", "bc")


ggphost25severity <- ggplot(dfpred, aes(x=hosts25F, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ geom_text(data=res3, aes(label = n, x = Group, y = median*100 +4, fill=Group),size = 3)+ylab("severity (% BA)") +xlab("hosts")+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Severity-agentpresence-hosts25.jpg"), plot=ggphost25severity, width=fig.width1,height=fig.width1,units="in")
```

```{r BB outbreak severity by hosts25, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[dfpred$affectedo=="affected",]
dfpred <- dfpred[!dfpred$hosts25=="none",]
...
res3 <- merge(res, res2, by.x="Group", by.y="hosts25F")
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "b", "b", "ab", "b", "b", "b")
res3[res3$Group == "PICO,\nPIEN,\n&\nABLA", "n"]<- ""

ggohost25severity <- ggplot(dfpred, aes(x=hosts25F, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ geom_text(data=res3, aes(label = n, x = Group, y = median*100 +4, fill=Group),size = 3)+ylab("severity (% BA)") +xlab("hosts")+theme(legend.position="none")

ggsave(here("Results/Figures/Figure-Severity-agentoutbreak-hosts25.jpg"), plot=ggohost25severity, width=fig.width1,height=fig.width1,units="in")
```


## Does the co-occurrence of multiple bark beetle species result in more severe tree mortality?

Finally, to determine if We then used Kruskall-Wallis test to determine if cumulative mortality differed with the identify of bark beetle species present. 

```{r multiple - BB pres by hosts , eval=F}
dfpred <-  df

dfpred <- dfpred[dfpred$hosts %in% c("PICO+PIEN",  "PICO+ABLA", "PIEN+ABLA",  "PICO+PIEN+ABLA"), ]
dfpred <-dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[!dfpred$agentsF == "none",]


results <- NULL
  dfsub <- dfpred[dfpred$hosts =="PICO+PIEN"  & dfpred$agents %in% c("MPB", "SB","MPB+SB"),]
  ...
  res <- merge(res, dfsub %>% group_by(agents) %>% summarise(median =median(propTBA), propTBA=max(propTBA), n=n()), by.x="Group", by.y="agents")
  res <- res[order(res$median),]
  res$Letter2 <- c("a", "a", "b")
  results <- rbind(results, res)
  
  dfsub <- dfpred[dfpred$hosts == "PICO+ABLA"  & dfpred$agentsF %in% c("MPB", "WBBB","MPB+WBBB"),]
  ...
  dfsub <- dfpred[dfpred$hosts == "PIEN+ABLA"  & dfpred$agentsF %in% c("SB", "WBBB","SB+WBBB"),]
  dfsub$agentsF <- as.factor(as.character(dfsub$agentsF))
  x <- kruskal.test(propTBA~agentsF, data=dfsub)
  DT <- dunnTest(propTBA~agentsF, data=dfsub,method="bh")
  res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.05)
  ...
  dfsub <- dfpred[dfpred$hosts == "PICO+PIEN+ABLA",]
  dfsub$agentsF <- as.factor(as.character(dfsub$agentsF))
  x <- kruskal.test(propTBA~agentsF, data=dfsub)
  DT <- dunnTest(propTBA~agentsF, data=dfsub,method="bh")
  res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.05)
  res$hosts <- "PICO+PIEN+ABLA"
  res <- merge(res, dfsub %>% group_by(agents) %>% summarise(median =median(propTBA), propTBA=max(propTBA), n=n()), by.x="Group", by.y="agents")
  res <- res[!res$Group =="none",]
  res <- res[order(res$median),]
  res$Letter2 <- c("a", "abc", "b", "b", "c", "d", "d")
  
  results <- rbind(results, res)
 
  
results$hosts <- factor(results$hosts,
levels=c("PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO & PIEN", "PICO & ABLA", "PIEN & ABLA", "PICO, PIEN, & ABLA"), ordered=T)
dfpred$hosts <- factor(dfpred$hosts, levels=c("PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO & PIEN", "PICO & ABLA", "PIEN & ABLA", "PICO, PIEN, & ABLA"), ordered=T)

dfpred$agentsF <- factor(dfpred$agentsF,
levels=c("MPB", "SB", "WBBB", "MPB+SB", "MPB+WBBB", "SB+WBBB", "MPB+SB+WBBB"), labels=c("MPB", "SB", "WBBB", "MPB\n&\nSB", "MPB\n&\nWBBB", "SB\n&\nWBBB", "MPB,\nSB,\n&\nWBBB"), ordered=T)
results$agentsF <- factor(results$Group,
levels=c("MPB", "SB", "WBBB", "MPB+SB", "MPB+WBBB", "SB+WBBB", "MPB+SB+WBBB"), labels=c("MPB", "SB", "WBBB", "MPB\n&\nSB", "MPB\n&\nWBBB", "SB\n&\nWBBB", "MPB,\nSB,\n&\nWBBB"), ordered=T)

ggmultipleseverity <- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsF,y=propTBA*100, fill=agentsF))+geom_boxplot(notch=F)+facet_grid(.~hosts, scales="free_x", space = "free_x")+theme(legend.position="none")+scale_fill_manual(values=rev(p2col)[-1])+ylab("Severity (% basal area)")+xlab("")+ylim(-5, 110) + geom_text(data=results, aes(label=Letter2), vjust=-1,size=4.5, col="black")+ geom_text(data=results, aes(label = n, y=-3),size = 3, col="black")+ylab("severity (% BA)")


ggsave(here("Results/Figures/Figure-MultipleSeverity-agents-hosts.jpg"), plot=ggmultipleseverity, width=fig.width2,height=fig.width1,units="in")

```

```{r multiple - BB outbreak by hosts , eval=F}
dfpred <-  df

dfpred <- dfpred[dfpred$hosts %in% c("PICO+PIEN",  "PICO+ABLA", "PIEN+ABLA",  "PICO+PIEN+ABLA"), ]
dfpred <-dfpred[dfpred$focalagent=="Any",]
dfpred <- dfpred[!dfpred$agentso == "none",]


results <- NULL
  dfsub <- dfpred[dfpred$hosts =="PICO+PIEN"  & dfpred$agentso %in% c("MPB", "SB","MPB+SB"),]
  dfsub$agentso <- as.factor(as.character(dfsub$agentso))
  x <- kruskal.test(propTBA~agentso, data=dfsub)
  res <- data.frame(agentsoF= c("MPB", "SB","MPB+SB"), p=NA)
  DT <- dunnTest(propTBA~agentso, data=dfsub,method="bh")
  res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.1)
  res$hosts <- "PICO+PIEN"

  res <- merge(res, dfsub %>% group_by(agentso) %>% summarise(median =median(propTBA), propTBA=max(propTBA), n=n()), by.x="Group", by.y="agentso")
  res <- res[order(res$median),]
  res$Letter2 <- c("a", "a", "b")
  results <- rbind(results, res)
  
  dfsub <- dfpred[dfpred$hosts == "PICO+ABLA"  & dfpred$agentso %in% c("MPB", "WBBB","MPB+WBBB"),]
  dfsub$agentso<- as.factor(as.character(dfsub$agentso))
  x <- kruskal.test(propTBA~agentso, data=dfsub)
  #DT <- dunnTest(propTBA~agentso, data=dfsub,method="bh")
  #res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.05)
  res <- data.frame(Group=c("MPB", "WBBB","MPB+WBBB"), Letter="", MonoLetter="")
  res$hosts <- "PICO+ABLA"
  ...
  dfsub <- dfpred[dfpred$hosts == "PIEN+ABLA"  & dfpred$agentso %in% c("SB", "WBBB","SB+WBBB"),]
  dfsub$agentso <- as.factor(as.character(dfsub$agentso))
  x <- kruskal.test(propTBA~agentso, data=dfsub)
  DT <- dunnTest(propTBA~agentso, data=dfsub,method="bh")
  res <- rcompanion::cldList(P.unadj ~ Comparison, data = DT$res, threshold = 0.05)
  ...
  res <- res[!res$Group =="none",]
  res <- res[order(res$median),]
  res$Letter2 <- c("a", "a", "ab", "abcd", "bc", "cd", "d")
...
dfpred$agentsoF <- factor(dfpred$agentso,
levels=c("MPB", "SB", "WBBB", "MPB+SB", "MPB+WBBB", "SB+WBBB", "MPB+SB+WBBB"), labels=c("MPB", "SB", "WBBB", "MPB\n&\nSB", "MPB\n&\nWBBB", "SB\n&\nWBBB", "MPB,\nSB,\n&\nWBBB"), ordered=T)
results$agentsoF <- factor(results$Group,
levels=c("MPB", "SB", "WBBB", "MPB+SB", "MPB+WBBB", "SB+WBBB", "MPB+SB+WBBB"), labels=c("MPB", "SB", "WBBB", "MPB\n&\nSB", "MPB\n&\nWBBB", "SB\n&\nWBBB", "MPB,\nSB,\n&\nWBBB"), ordered=T)

ggmultipleseverity <- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsoF,y=propTBA*100, fill=agentsoF))+geom_boxplot(notch=F)+facet_grid(.~hosts, scales="free_x", space = "free_x")+theme(legend.position="none")+scale_fill_manual(values=rev(p2col)[-1])+ylab("Severity (% basal area)")+xlab("")+ylim(-5, 110) + geom_text(data=results, aes(label=Letter2), vjust=-1,size=4.5, col="black")+ geom_text(data=results, aes(label = n, y=-3),size = 3, col="black")+ylab("severity (% BA)")


ggsave(here("Results/Figures/Figure-MultipleSeverity-agentso-hosts.jpg"), plot=ggmultipleseverity, width=fig.width2,height=fig.width1,units="in")

```

```{r multiple - BB pres by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[!dfpred$suitablehosts == "none", ]
dfpred <- dfpred[dfpred$affected=="affected",]
...
results <- NULL
  ...
  res <- res[!res$Group =="none",]
  res <- res[order(res$median),]
  res$Letter2 <- c("ab", "b", "ab", "ab", "bc", "cd", "d")
...
ggsave(here("Results/Figures/Figure-MultipleSeverity-agent-suitablehosts.jpg"), plot=ggmultipleseverity, width=fig.width2,height=fig.width1,units="in")
```
...
Models Table \@ref(tab:TestPerformance)

## Do stand conditions suitable for multiple outbreaks commonly co-occur?

```{r describe frequency of hosts and agents}
## Hosts
...
Across the 10,439 FIA plots with at least one of the target host species, 38% (n=3,915) contained two species and 19% (n=1,935) three species (Fig. \@ref(fig:FigHostFreq)). Subalpine fir was most likely to occur with other target host species (50% of plots with multiple hosts), followed by Engelmann spruce (45% of plots with multiple hosts), and lodgepole pine (36% of plots with multiple hosts). 

However, random forest models predicted that 70% of plots (n=7,292) were suitable for bark beetle infestation and of these plots only 31% (n=1,855) were suitable for more than one bark beetle species (Fig. \@ref(fig:FigHostFreq)). Again, conditions suitable for bark beetle attack in subalpine fir were most likely to co-occur with conditions suitable for bark beetle attack in another tree species (17% of plots).

```{r FigHostFreq, fig.cap = "The frequency of plots by host identity. Gray bars indicate host species were present (i.e., at least one individual of the focal species was present within the plot). Black bars indicate stand structure and composition conditions were suitbale to bark beetle outbreak in focal tree species. For host identities, PICO is lodgepole pine, PIEN is Engelmann spruce, and ABLA is subalpine fir.", out.width = "244.8pt"}
knitr::include_graphics( here("Results/Figures/FigureHostCo-outbreak.jpg"))
```

## Does the occurrence or severity of cumulative bark beetle activity increase with the host species richness?

Given stand conditions suitable to bark beetle outbreak, stands with lodgepole and spruce, lodgepole and fir, or all three hosts were most likely to experience outbreak (Fig. \@ref(fig:FigBBO)). Stands with only subalpine fir were least likely to experience outbreak, followed by stands with only spruce and their combination. 

```{r FigBBO, fig.cap = "The proportion of plots affected by bark beetles by the identity of suitable hosts. Letters above bars indicate significant (p<0.05) difference between groups, as determined using pairwise proportion tests. For host identities, PICO is lodgepole pine, PIEN is Engelmann spruce, and ABLA is subalpine fir.", out.width = "244.8pt"}
knitr::include_graphics( here("Results/Figures/FigureOccurrencebySuitablehosts2-outbreak.jpg"))
```

The effect of the number of suitable hosts on the severity of bark beetle infestation depended upon the identity of host species (Fig. \@ref(fig:FigSeverity). Stands with only subalpine fir experienced the lowest severity bark beetle infestation. In contrast, stands with only lodgepole pine and Engelmann spruce and their combination, experienced the highest rates of bark beetle infestation. Stands with subalpine fir in combination with other hosts, experienced intermediate rates of bark beetle infestation.

```{r FigSeverity, fig.cap= "Boxplots illustrating the severity of bark beetle infestation by the identity of host species susceptible to bark beetle outbreak. For host identities, PICO is lodgepole pine, PIEN is Engelmann spruce, and ABLA is subalpine fir. The bottom and top limits of each box are the lower and upper quartiles, respectively; the thick black line within the box is the median; error bars equal ±1.5 times the interquartile range; and points denote outliers, values outside ±1.5 times the interquartile range. Letters above boxes indicate significant (p<0.05) difference between groups, as determined using a nonparametric Dunn test.", out.width = "244.8pt"}
knitr::include_graphics( here("Results/Figures/FigureSevertitybySuitablehosts-outbreak.jpg"))
```

## Does the co-occurrence of multiple bark beetle species result in more severe cumulative bark beetle mortality?

Across all 10,439 FIA plots, only 5.3% (n=539) were affected by outbreaks of multiple bark beetle species.  
...
knitr::include_graphics( here("Results/Figures/FigureHotSpotSeverity-outbreak.jpg"))
```
...
## Supplement
### Appendix I: Random Forest Modeling

```{r Performance occurrence}
MPBtp <- read.csv(here("Results/RF-testperformance-agentMPB-occurrence-SMOTE.csv"))
MPBtp$focalagent <- "MPB"
SBtp <- read.csv(here("Results/RF-testperformance-agentSB-occurrence-SMOTE.csv"))
SBtp$focalagent <- "SB"
WBBBtp <- read.csv(here("Results/RF-testperformance-agentWBBB-occurrence-SMOTE.csv"))
WBBBtp$focalagent <- "WBBB"

tpdat <- rbind(rbind(MPBtp, SBtp), WBBBtp)
tpdat$.estimate <-round(tpdat$.estimate, digits=2)
tpdat2 <- tpdat[,c(".metric", "focalagent", ".estimate")] %>% spread(key=focalagent, value=.estimate)
colnames(tpdat2) <- c("Metric","MPB", "SB", "WBBB")
tpdat2$Metric <- c("ROC AUC", "sensitivity", "specificity")
kable(tpdat2,booktabs = TRUE,caption="Accuracy statistics from Random Forest models prediciting the presence/absence of the mountain pine beetle (MPB) in lodgepole pine, spruce beetle (SB) in Engelmann spruce, and western balsam bark beetle (WBBB) in subalpine fir. ROC AUC is the area under the reciever operating characteristic curve.")
```

```{r Performance outbreak}
MPBtp <- read.csv(here("Results/RF-testperformance-agentMPBo-occurrence-SMOTE.csv"))
MPBtp$focalagent <- "MPB"
SBtp <- read.csv(here("Results/RF-testperformance-agentSBo-occurrence-SMOTE.csv"))
SBtp$focalagent <- "SB"
WBBBtp <- read.csv(here("Results/RF-testperformance-agentWBBBo-occurrence-SMOTE.csv"))
WBBBtp$focalagent <- "WBBB"
...
colnames(tpdat2) <- c("Metric","MPB", "SB", "WBBB")
tpdat2$Metric <- c("ROC AUC", "sensitivity", "specificity")
kable(tpdat2,booktabs = TRUE,caption="Accuracy statistics from Random Forest models prediciting the presence/absence of outbreaks of mountain pine beetle (MPB) in lodgepole pine, spruce beetle (SB) in Engelmann spruce, and western balsam bark beetle (WBBB) in subalpine fir. ROC AUC is the area under the reciever operating characteristic curve.")
```

```{r plot confusion matrices, eval=F}
...
MPBct <- read.csv(here("Results/RF-confusiontable-agentMPBo-occurrence-SMOTE.csv"))
SBct <- read.csv(here("Results/RF-confusiontable-agentSBo-occurrence-SMOTE.csv"))
WBBBct <- read.csv(here("Results/RF-confusiontable-agentWBBBo-occurrence-SMOTE.csv"))

BBct <- data.frame(agent=c("MPB", "SB", "WBBB"), sensitivity=c(MPBct[2,2]/sum(MPBct[,2]), SBct[2,2]/sum(SBct[,2]), WBBBct[2,2]/sum(WBBBct[,2])), accuracy= c((MPBct[2,2]+MPBct[1,1])/sum(MPBct), (SBct[2,2]+SBct[1,1])/sum(SBct), (WBBBct[2,2]+WBBBct[1,1])/sum(WBBBct)))
BBct$var <- "outbreak"

BBct <- rbind(rbind(BBct1, BBct))

ggconfused <- ggplot(BBct, aes(x=agent, y=sensitivity, fill=var))+geom_bar(stat="identity", position="dodge")+theme(legend.position = "none") + ggplot(BBct, aes(x=agent, y=accuracy, fill=var))+geom_bar(stat="identity", position="dodge")+plot_layout(nrow=2, guides="collect")


ggsave(here("Results/Figures/Confusion-outbreak.jpg"), plot=ggconfused, width=fig.width1,height=fig.width1,units="in")
```

```{r plot variable importance occurrence, eval=F}
MPBvimp <- read.csv(here("Results/RF-vimp-agentMPB-occurrence-SMOTE.csv"))
MPBvimp$focalagent <- "MPB"
MPBvimp$variable <- c("BA", "QMD", "BA dominance")
MPBvimp$scale <-MPBvimp$ranger_obj.variable.importance / sum(MPBvimp$ranger_obj.variable.importance )*100

SBvimp <- read.csv(here("Results/RF-vimp-agentSB-occurrence-SMOTE.csv"))
SBvimp$focalagent <- "SB"
SBvimp$variable <- c("BA", "QMD", "BA dominance")
SBvimp$scale <-SBvimp$ranger_obj.variable.importance / sum(SBvimp$ranger_obj.variable.importance )*100

WBBBvimp <- read.csv(here("Results/RF-vimp-agentWBBB-occurrence-SMOTE.csv"))
WBBBvimp$focalagent <- "WBBB"
WBBBvimp$variable <- c("BA", "QMD", "BA dominance")
WBBBvimp$scale <-WBBBvimp$ranger_obj.variable.importance / sum(WBBBvimp$ranger_obj.variable.importance )*100

vimpdat <- rbind(rbind(MPBvimp, SBvimp), WBBBvimp)
colnames(vimpdat) <- c("importance","focalagent", "variable", "scale")
vimpdat$focalagent <- factor(vimpdat$focalagent)  
vimpdat$variable <- factor(vimpdat$variable)  
vimpplot <- ggplot(vimpdat, aes(x=variable, y=scale, fill=focalagent))+geom_bar(stat="identity", position="dodge")+theme(legend.position="bottom", legend.title=element_blank(), axis.title.y = element_blank())+coord_flip()+ylab("variable importance\n(% contribution to overall model fit)")+scale_fill_manual(values=p1col[1:3])

ggsave(here("Results/Figures/VIMP-occurrence.jpg"), plot=vimpplot, width=fig.width1,height=fig.width1,units="in")
```

```{r plot variable importance outbreak, eval=F}
MPBvimp <- read.csv(here("Results/RF-vimp-agentMPBo-occurrence-SMOTE.csv"))
MPBvimp$focalagent <- "MPB"
MPBvimp$variable <- c("BA", "QMD", "BA dominance")
MPBvimp$scale <-MPBvimp$ranger_obj.variable.importance / sum(MPBvimp$ranger_obj.variable.importance )*100

SBvimp <- read.csv(here("Results/RF-vimp-agentSBo-occurrence-SMOTE.csv"))
SBvimp$focalagent <- "SB"
SBvimp$variable <- c("BA", "QMD", "BA dominance")
SBvimp$scale <-SBvimp$ranger_obj.variable.importance / sum(SBvimp$ranger_obj.variable.importance )*100

WBBBvimp <- read.csv(here("Results/RF-vimp-agentWBBBo-occurrence-SMOTE.csv"))
WBBBvimp$focalagent <- "WBBB"
WBBBvimp$variable <- c("BA", "QMD", "BA dominance")
WBBBvimp$scale <-WBBBvimp$ranger_obj.variable.importance / sum(WBBBvimp$ranger_obj.variable.importance )*100
...
ggsave(here("Results/Figures/VIMP-outbreak.jpg"), plot=vimpplot, width=fig.width1,height=fig.width1,units="in")
```

```{r partial dependence occurrence, eval=F}
df <- read.csv( here("Data/Processed/FIAdat-recent-re.csv"))
df$focalagent <- factor(df$focalhost, levels= c("PICO", "PIEN", "ABLA", "Allhosts"), labels=c("MPB", "SB", "WBBB", "Any"), ordered=T)
...
x <- x[,c("QMD.host", "propBA", "BA.host", "focalagent", "affected")]
x$affected01 <- ifelse(x$affected=="affected", 1, 0)
...
pBA1 <- ggplot(ppdat[ppdat$var=="BA" & ppdat$method=="PD",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(0, 60)

pBA2 <- ggplot(xm[xm$var=="BA",],aes(x=value, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot(se=F)+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("bark beetle")+xlab("basal area")+xlim(0, 60)

pQMD1 <- ggplot(ppdat[ppdat$var=="QMD"& ppdat$method=="PD",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(10, 100)

pQMD2 <- ggplot(xm[xm$var=="QMD",],aes(x=value, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot()+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("")+xlab("QMD")+xlim(10, 100)

ppropBA1 <- ggplot(ppdat[ppdat$var=="propBA" & ppdat$method=="PD",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(0, 100)

ppropBA2 <- ggplot(xm[xm$var=="propBA",],aes(x=value*100, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot()+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("")+xlab("% BA")+xlim(0, 100)
...
ggsave(here("Results/Figures/PartialDependence-occurrence.jpg"), plot=ppplot, width=fig.width2,height=fig.width1,units="in")
```

```{r partial dependence outbreak, eval=F}
df <- read.csv( here("Data/Processed/FIAdat-recent-re.csv"))
df$focalagent <- factor(df$focalhost, levels= c("PICO", "PIEN", "ABLA", "Allhosts"), labels=c("MPB", "SB", "WBBB", "Any"), ordered=T)

MPBpp <- read.csv(here("Results/RF-partialdependence-agentMPBo-occurrence-SMOTE.csv"))
MPBpp$focalagent <- "MPB"
SBpp <- read.csv(here("Results/RF-partialdependence-agentSBo-occurrence-SMOTE.csv"))
SBpp$focalagent <- "SB"
WBBBpp <- read.csv(here("Results/RF-partialdependence-agentWBBBo-occurrence-SMOTE.csv"))
WBBBpp$focalagent <- "WBBB"
...
ggsave(here("Results/Figures/PartialDependence-outbreak.jpg"), plot=ppplot, width=fig.width2,height=fig.width1,units="in")
```

```{r ale occurrence, eval=F}
df <- read.csv( here("Data/Processed/FIAdat-recent-re.csv"))
df$focalagent <- factor(df$focalhost, levels= c("PICO", "PIEN", "ABLA", "Allhosts"), labels=c("MPB", "SB", "WBBB", "Any"), ordered=T)
...
pBA1 <- ggplot(ppdat[ppdat$var=="BA" & ppdat$method=="ALE",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(0, 60)

pBA2 <- ggplot(xm[xm$var=="BA",],aes(x=value, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot(se=F)+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("bark beetle")+xlab("basal area")+xlim(0, 60)

pQMD1 <- ggplot(ppdat[ppdat$var=="QMD"& ppdat$method=="ALE",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(10, 100)

pQMD2 <- ggplot(xm[xm$var=="QMD",],aes(x=value, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot()+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("")+xlab("QMD")+xlim(10, 100)

ppropBA1 <- ggplot(ppdat[ppdat$var=="propBA" & ppdat$method=="ALE",], aes(x=value, y=yhat, col=focalagent))+geom_smooth(se=F)+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )+xlim(0, 100)

ppropBA2 <- ggplot(xm[xm$var=="propBA",],aes(x=value*100, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot()+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("")+xlab("% BA")+xlim(0, 100)

ppplot <- pBA1  + pQMD1 +theme(axis.title.y=element_blank(),axis.text.y=element_blank()) +ppropBA1 +theme(axis.title.y=element_blank(),axis.text.y=element_blank())+ pBA2 + pQMD2 +theme(axis.text.y=element_blank())  + ppropBA2 +theme(axis.text.y=element_blank()) + plot_layout(ncol=3, heights=c(2,1))

ggsave(here("Results/Figures/ALE-occurrence.jpg"), plot=ppplot, width=fig.width2,height=fig.width1,units="in")
```

```{r ale outbreak, eval=F}
df <- read.csv( here("Data/Processed/FIAdat-recent-re.csv"))
df$focalagent <- factor(df$focalhost, levels= c("PICO", "PIEN", "ABLA", "Allhosts"), labels=c("MPB", "SB", "WBBB", "Any"), ordered=T)
...
ggsave(here("Results/Figures/ALE-outbreak.jpg"), plot=ppplot, width=fig.width2,height=fig.width1,units="in")
```
...
```{r partialFig, fig.cap="The associations between the probability of occurrence of the mountain pine beetle (MPB; red), spruce beetle (SB; blue); and western balsam bark beetle (WBBB; yellow) and three common measuree of stand suceptibility to bark beetl beetle outbreak, basal area (BA), quadratic mean diameter (QMD), and host dominance (% BA). The top row illustrates smoothed partial dependence results from Random Forest modeling, while in the bottom row boxplots show the distribution of values for stands affected (dark colors) and unaffected (light colors) by each bark beetle species."}
knitr::include_graphics(here("Results/Figures/PartialDependence-outbreak.jpg"))
```
...
Fig.file <- here("Results/Figures/Figure-Map-Suitableplots-outbreak.jpg")
jpeg(Fig.file, width=fig.width1.5, height=fig.width2/2, units="in", res=300)
tmap_arrange(map1, map2,map3, map4, map5, map6, map7,legend.map, nrow=2)
whatever <- dev.off()
```

```{r FigSuitableMap, fig.cap="a caption", out.width = "504.8pt", eval=F}
knitr::include_graphics(here("Results/Figures/Figure-Map-Suitableplots-outbreak.jpg"))
```
...
Fig.file <- here("Results/Figures/Figure-Studyarea-outbreak.jpg")
jpeg(Fig.file, width=fig.width1.5,height=fig.width1.5,units="in", res=300)
sa.tmap 
whatever <- dev.off()
```

```{r FigSA, fig.cap = "The distribution of host species presence across the study area. Data are from the Individual Tree Species Atlas (Ellenwood et al. 2015) and represent conditions in ca. 2002.", out.width = "322.56pt", eval=F}
knitr::include_graphics( here("Results/Figures/Figure-Studyarea-outbreak.jpg"))
```

```{r severity by host presence, eval=F}
dfpred <- df
...
ggsave(here("Results/Figures/FigureSevertitybyPresence-outbreak.jpg"), plot=ggseverity, width=fig.width2,height=fig.width1*0.66,units="in")
```

```{r severity by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred$suitablehosts <- factor(dfpred$suitablehosts, levels=levels(dfpred$suitablehosts)[-1], labels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"))
...
ggsave(here("Results/Figures/FigureSevertitybySuitablehosts-outbreak.jpg"), plot=ggseverity, width=fig.width2,height=fig.width1*0.66,units="in")
```

```{r occurrence by host presence, eval=F}
dfpred <- df
...
ggsave( here("Results/Figures/FigureOccurrencebyPresence-outbreak.jpg"), plot=ggoccurrence,  width=fig.width2, height=fig.width1*0.5,units="in")
...
```{r occurrence by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred$suitablehosts <- factor(dfpred$suitablehosts, levels=levels(dfpred$suitablehosts)[-1], labels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"))
...
ggsave( here("Results/Figures/FigureOccurrencebySuitablehosts-outbreak.jpg"), plot=ggoccurrence,  width=fig.width2, height=fig.width1*0.5,units="in")
