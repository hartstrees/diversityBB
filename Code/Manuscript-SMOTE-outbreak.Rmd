...
df$nhosts25F <-factor(df$nhosts, levels=1:3, labels=c("one", "two", "three"), ordered=T)
df$hosts25 <- factor(df$hosts, levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), ordered=T)
df$hosts25F <- factor(df$hosts, levels=c("PICO", "PIEN", "ABLA", "PICO+PIEN", "PICO+ABLA", "PIEN+ABLA", "PICO+PIEN+ABLA"), labels=c("PICO", "PIEN", "ABLA", "PICO & PIEN", "PICO & ABLA", "PIEN & ABLA", "PICO, PIEN, & ABLA"), ordered=T)
...
```{r fit MPB RF model, eval=F,fig.show='hide'}
response <- "agentMPB"
...
```{r fit MPB RF model outbreak, eval=F}
response <- "agentMPBo"
vars <- c("BA.PICO", "QMD.PICO", "propBA.PICO") # predictor variables
datBB.c <- df[df$focalhost=="Any",] # dataframe containing response and predictors (here i had to subset it because of how my data was arranged)
...
pd <- NULL
for(z in vars.final){
  pdz <- partial(ranger_obj, train=datz.train, pred.var=z, which.class=strsplit(strsplit(response, split="agent")[[1]][2], split="o")[[1]][1], prob=T)
  pdz$var <- z
  colnames(pdz) <- c("value","yhat","var")
  pd <- rbind(pd, pdz)
}
...
```{r fit SB RF model outbreak, eval=F}
response <- "agentSBo"
vars <- c("BA.PIEN", "QMD.PIEN", "propBA.PIEN") # predictor variables
datBB.c <- df[df$focalhost=="Any",] # dataframe containing response and predictors (here i had to subset it because of how my data was arranged)
...
```{r fit WBBB RF model outbreak, eval=F}
response <- "agentWBBBo"
vars <- c("BA.ABLA", "QMD.ABLA", "propBA.ABLA") # predictor variables
datBB.c <- df[df$focalhost=="Any",] # dataframe containing response and predictors (here i had to subset it because of how my data was arranged)
...
ggplot(hostz, aes(x=hosts, y=n, fill=var))+geom_bar(stat="identity", position = position_dodge2(width = 0.9, preserve = "single"))+theme(legend.position="bottom")+xlab("")+ylab("number of plots")
...
ggplot(resp.hosts, aes(x=hosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")
```

```{r outbreak occurrence by hosts, eval=T}
...
ggplot(resp.suitablehosts, aes(x=suitablehosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")

```

```{r outbreak occurrence by suitable hosts}
## Suitable hosts
...
ggplot(resp.suitablehostso, aes(x=suitablehosts,y=prop))+geom_bar(stat="identity", width=0.75)+geom_text(aes(label=Letter2), vjust=-1,size=3)+ylim(0,1)+xlab("suitable hosts")+ylab("proportion affected")+scale_fill_manual(values=colorspace::adjust_transparency(p2col, alpha=0.6))+theme(legend.position="none")
```
...
ggsi <- ggplot(dfpred, aes(x=hosts, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ ylab("severity (% BA)") +xlab("hosts")+theme(legend.position="none")
 
ggsave(here("Results/Figures/FigureSeverityby-AllHosts-outbreak.jpg"), plot=ggsi, width=fig.width1,height=fig.width1,units="in")
```
...
res3 <- res3[order(res3$median),]
res3$Letter2 <- c("a", "b", "b", "c", "bc", "c", "c")

ggss <- ggplot(dfpred, aes(x=suitablehosts, y=propTBA*100))+geom_boxplot(notch=F,fill="lightgrey")+geom_text(data=res3, aes(label = Letter2, x = Group, y = 5 + max*100, fill=Group))+ ylab("severity (% BA)") +xlab("suitable hosts")+theme(legend.position="none")
 
ggsave(here("Results/Figures/FigureSeverityby-Suitablehosts-outbreak.jpg"), plot=ggss, width=fig.width1,height=fig.width1,units="in")
```

```{r severity combo, eval=F}
ggs <- ggsi+theme(axis.title.x = element_blank(), axis.text.x = element_blank()) + ggss + plot_layout(ncol=1)

ggsave( here("Results/Figures/FigureSeverityby-Hosts-outbreak.jpg"), plot=ggs,  width=fig.width1, height=fig.width1*2,units="in")
```

```{r determine co-occurrence of agents by hosts, eval=F}
...
```{r determine co-occurrence of agents by suitable hosts, eval=F}
...
```{r multiple agents by hosts, eval=F}
dfpred <-  df
...
ggmultipleseverity <- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsF,y=propTBA*100, fill=agentsF))+geom_boxplot(notch=F)+facet_grid(.~hosts, scales="free_x", space = "free_x")+theme(legend.position="none")+scale_fill_manual(values=rev(p2col)[-1])+ylab("Severity (% basal area)")+xlab("")+ylim(0,110)+geom_text(data=results, aes(label=Letter2), vjust=-1,size=4.5, col="black")+theme(axis.text.x=element_blank(), axis.title.x=element_blank(), axis.ticks = element_blank(), plot.margin = unit(c(4.5,4.5,0,4.5), "pt"))

ggmultipelcount<- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsF, fill=agentsF))+geom_bar()+facet_grid(.~hosts, scales="free", space = "free_x")+theme(legend.position="none")+scale_fill_manual(values=rev(p2col)[-1])+xlab("")+ylab("frequency")+theme(strip.background = element_blank(), strip.text.x = element_blank(), plot.margin = unit(c(3.5,4.5,0,4.5), "pt"))

ggmulti <- ggmultipleseverity + ggmultipelcount +plot_layout(ncol=1, heights=c(1,0.5))
ggsave(here("Results/Figures/FigureSeverityMultiple-outbreak-allhosts.jpg"), plot=ggmulti  , width=fig.width2,height=fig.width1.5,units="in")

ggmultipleseverity <- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsF,y=propTBA*100, fill=agentsF))+geom_boxplot(notch=F)+facet_grid(.~hosts, scales="free_x", space = "free_x")+theme(legend.position="none")+scale_fill_manual(values=rev(p2col)[-1])+ylab("Severity (% basal area)")+xlab("Bark beetle species present")+ylim(0,110)+geom_text(data=results, aes(label=Letter2), vjust=-1,size=4.5, col="black")

ggsave(here("Results/Figures/FigureHotSpotSeverity-outbreak-allhosts.jpg"), plot=ggmultipleseverity, width=fig.width2,height=fig.width1*0.75,units="in")

```

```{r multiple agents by suitable hosts, eval=F}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[!dfpred$suitablehosts == "none", ]
...
results <- NULL
  ...
  res <- res[!res$Group =="none",]
  res <- res[order(res$median),]
  res$Letter2 <- c("a", "abc", "ab", "ab", "b", "c", "c")
...
ggmulti <- ggmultipleseverity + ggmultipelcount +plot_layout(ncol=1, heights=c(1,0.5))
ggsave(here("Results/Figures/FigureSeverityMultiple-outbreak-suitablehosts.jpg"), plot=ggmulti  , width=fig.width2,height=fig.width1.5,units="in")
...
ggsave(here("Results/Figures/FigureHotSpotSeverity-outbreak-suitablehosts.jpg"), plot=ggmultipleseverity, width=fig.width2,height=fig.width1*0.75,units="in")
...
rf_workflow_final_fit<- rf_workflow_final %>% last_fit(splitit, metrics=metric_set(roc_auc, sens, spec)) #emulates the process where, after determining the best model, the final fit on the entire training set is needed and is then evaluated on the test set. 
test_performance <- rf_workflow_final_fit %>% collect_metrics()
write-outbreak.csv(test_performance, here(paste0("Results/RF-testperformance-", response, "-occurrence-BALANCED-outbreak.csv")), row.names=F)
  
# generate a confusion matrix for test set
test_predictions <- rf_workflow_final_fit %>% collect_predictions()
confusion <- test_predictions %>% conf_mat(response,estimate = .pred_class)

write-outbreak.csv(confusion$table, here(paste0("Results/RF-confusiontable-", response, "-occurrence-BALANCED-outbreak.csv")), row.names=F)
...
write-outbreak.csv(x, here(paste0("Results/RF-pred-", response, "-occurrence-BALANCED-outbreak.csv")), row.names=F)
          
x <- as.data.frame(ranger_obj$variable.importance)
write-outbreak.csv(x, here(paste0("Results/RF-vimp-", response, "-occurrence-BALANCED-outbreak.csv")), row.names=F)
...
write-outbreak.csv(pd,  here(paste0("Results/RF-partialdependence-", response, "-occurrence-BALANCED-outbreak.csv")), row.names=F)
```
...
rf_workflow_final_fit<- rf_workflow_final %>% last_fit(splitit, metrics=metric_set(roc_auc, sens, spec)) #emulates the process where, after determining the best model, the final fit on the entire training set is needed and is then evaluated on the test set. 
test_performance <- rf_workflow_final_fit %>% collect_metrics()
write-outbreak.csv(test_performance, here(paste0("Results/RF-testperformance-", response, "-occurrence-WEIGHTED-outbreak.csv")), row.names=F)
  
# generate a confusion matrix for test set
test_predictions <- rf_workflow_final_fit %>% collect_predictions()
confusion <- test_predictions %>% conf_mat(response,estimate = .pred_class)

write-outbreak.csv(confusion$table, here(paste0("Results/RF-confusiontable-", response, "-occurrence-WEIGHTED-outbreak.csv")), row.names=F)
...
x <- cbind(x, predict(ranger_obj, datBB.c)$predictions[,2])
colnames(x) <- c("PLT_CN", "SBpred")
write-outbreak.csv(x, here(paste0("Results/RF-pred-", response, "-occurrence-WEIGHTED-outbreak.csv")), row.names=F)
          
x <- as.data.frame(ranger_obj$variable.importance)
write-outbreak.csv(x, here(paste0("Results/RF-vimp-", response, "-occurrence-WEIGHTED-outbreak.csv")), row.names=F)
...
write-outbreak.csv(pd,  here(paste0("Results/RF-partialdependence-", response, "-occurrence-WEIGHTED-outbreak.csv")), row.names=F)
```
...
MPBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentMPB-occurrence-SMOTE-outbreak.csv"))
SBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentSB-occurrence-SMOTE-outbreak.csv"))
WBBBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentWBBB-occurrence-SMOTE-outbreak.csv"))
...
MPBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentMPB-occurrence-BALANCED-outbreak.csv"))
SBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentSB-occurrence-BALANCED-outbreak.csv"))
WBBBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentWBBB-occurrence-BALANCED-outbreak.csv"))

BBct <- data.frame(agent=c("MPB", "SB", "WBBB"), sensitivity=c(MPBct[2,2]/sum(MPBct[,2]), SBct[2,2]/sum(SBct[,2]), WBBBct[2,2]/sum(WBBBct[,2])), accuracy= c((MPBct[2,2]+MPBct[1,1])/sum(MPBct), (SBct[2,2]+SBct[1,1])/sum(SBct), (WBBBct[2,2]+WBBBct[1,1])/sum(WBBBct)))
BBct$approach <- "Balanced"
BBct2  <- BBct

MPBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentMPB-occurrence-WEIGHTED-outbreak.csv"))
SBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentSB-occurrence-WEIGHTED-outbreak.csv"))
WBBBct <- read-outbreak.csv(here("Results/RF-confusiontable-agentWBBB-occurrence-WEIGHTED-outbreak.csv"))
...
```{r partial dependence, eval=F}
MPBpp <- read.csv(here("Results/RF-partialdependence-agentMPB-occurrence-outbreak-SMOTE.csv"))
MPBpp$focalagent <- "MPB"
SBpp <- read.csv(here("Results/RF-partialdependence-agentSB-occurrence-outbreak-SMOTE.csv"))
SBpp$focalagent <- "SB"
WBBBpp <- read.csv(here("Results/RF-partialdependence-agentWBBB-occurrence-outbreak-SMOTE.csv"))
...
```{r TestPerformance}
MPBtp <- read.csv(here("Results/RF-testperformance-agentMPB-occurrence-outbreak-SMOTE.csv"))
MPBtp$focalagent <- "MPB"
SBtp <- read.csv(here("Results/RF-testperformance-agentSB-occurrence-outbreak-SMOTE.csv"))
SBtp$focalagent <- "SB"
WBBBtp <- read.csv(here("Results/RF-testperformance-agentWBBB-occurrence-outbreak-SMOTE.csv"))
...
kable(tpdat2,booktabs = TRUE,caption="A caption")
```
