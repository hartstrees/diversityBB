...
x <- x[,c("QMD.host", "propTBA", "BA.host", "focalagent", "affected01")]
x$affected01 <- ifelse(x$affected01=="affected", 1, 0)

xm <- melt(x, id.vars=c("focalagent", "affected01"))
colnames(xm) <- c("focalagent", "yhat", "var", "value")
xm$var <- factor(xm$var, levels=c("QMD.host", "propTBA", "BA.host"), labels=c("QMD", "propBA", "BA"))
xm$focalagent <- as.character(xm$focalagent)
p <- ggplot(ppdat[ppdat$var=="BA",], aes(x=value, y=yhat, col=focalagent))+geom_smooth()+scale_color_manual(values=p1col)+geom_point(data=xm[xm$var=="BA",], size=0, )+ylim(0.1,0.6)
ggplot(xm[xm$var=="BA",],aes(x=value, y=yhat, col=focalagent)



ggplot(xm, aes(x=focalagent, y=value))
```
