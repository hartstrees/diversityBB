...
#### Selection of plots
We selected all plots that were part of the annnual inventory and where all subplots were inventoried. 
...
f19 <- stars::read_stars("/Volumes/GoogleDrive/My Drive/RESEARCH/GitHub/bbdiversity/Data/Spatial/ITSPM2002-subalpine/f19/w001001.adf", proxy=F)
f93 <- stars::read_stars("/Volumes/GoogleDrive/My Drive/RESEARCH/GitHub/bbdiversity/Data/Spatial/ITSPM2002-subalpine/f93/w001001.adf", proxy=F)
f108 <- stars::read_stars("/Volumes/GoogleDrive/My Drive/RESEARCH/GitHub/bbdiversity/Data/Spatial/ITSPM2002-subalpine/f108/w001001.adf", proxy=F)
...
```{r select plots with large & abundant, eval=T}
PICO.BA.threshold <- quantile(df.sub[df.sub$agentMPB=="MPB", ]$BA.PICO, 0.10)
PIEN.BA.threshold <- quantile(df.sub[df.sub$agentSB=="SB", ]$BA.PIEN, 0.10)
ABLA.BA.threshold <- quantile(df.sub[df.sub$agentWBBB=="WBBB", ]$BA.ABLA, 0.10)

PICO.QMD.threshold <- quantile(df.sub[df.sub$agentMPB=="MPB", ]$QMD.PICO, 0.10)
PIEN.QMD.threshold <- quantile(df.sub[df.sub$agentSB=="SB", ]$QMD.PIEN, 0.10)
ABLA.QMD.threshold <- quantile(df.sub[df.sub$agentWBBB=="WBBB", ]$QMD.ABLA, 0.10)
n
# PICO, PIEN & ABLA
...
df.sub.PICO.PIEN.ABLA$focalhosts2 <- "PICO, PIEN & ABLA"
...
df.sub.PICO.PIEN$focalhosts2 <- "PICO & PIEN"
...
df.sub.PICO.ABLA$focalhosts2 <- "PICO & ABLA"
...
df.sub.PIEN.ABLA$focalhosts2 <- "PIEN & ABLA"
...
## Affected hosts
affectedhosts2 <- df.focal2 %>% group_by(focalhosts2, affected) %>% summarise(n = n())%>% mutate(freq = prop.table(n))
...
MPBnhosts2 <- df.focal2 %>% group_by(nhosts2, agentMPB) %>% summarise(n = n())%>% mutate(freq = prop.table(n))
