...
### Import libraries
ipak(c('tidyverse', 'tidymodels', 'reshape2', 'here', 'rgdal', 'sp', 'raster', 'stars', 'sf', 'prism', 'tigris', "tmap", 'fasterize', 'ggplot2','RColorBrewer','ggsci', 'patchwork', 'colorspace', 'grid', 'knitr',  'kableExtra', 'bookdown',  'pdp', 'ranger', 'VSURF', 'themis', 'foreach'))
...
```{r determine co-occurrence of hosts}
...
### Determine suitable hosts
Construct random forest models using synthetic minority oversampling technique (SMOTE)
...
splitit <- initial_split(datBB.c, prop=.7, strata=response)
datz.train <- training(splitit)
datz.test <- testing(splitit)
datz.train.cv <- vfold_cv(datz.train, strata=response, v=10)
...
# set model
rf_model <- rand_forest() %>% set_args( trees=tune(), min_n=tune()) %>% set_engine("ranger", importance = "permutation", probability=T, seed=913, sample.fraction=wts) %>% set_mode("classification") 
...
rf_tune_results <- rf_workflow %>% tune_grid(resamples = datz.train.cv,  grid = tgrid, metrics = metric_set(roc_auc, accuracy, sens, spec))
...
rf_model_final <- rand_forest() %>% set_args(trees=param_best$trees, min_n=param_best$min_n, splitrule="gini") %>% set_engine("ranger", importance = "permutation", probability=T, sample.fraction=wts) %>% set_mode("classification") 

rf_workflow_final <- workflow() %>% add_recipe(BBoccurrence_recipe_final) %>% add_model(rf_model_final)
rf_workflow_final_fit<- rf_workflow_final %>% last_fit(splitit, metrics=metric_set(roc_auc, accuracy, sens, spec)) #emulates the process where, after determining the best model, the final fit on the entire training set is needed and is then evaluated on the test set. 
test_performance <- rf_workflow_final_fit %>% collect_metrics()
write.csv(test_performance, here(paste0("Results/RF-testperformance-", response, "-occurrence-2.csv")), row.names=F)
...
write.csv(confusion$table, here(paste0("Results/RF-confusiontable-", response, "-occurrence-2.csv")), row.names=F)
...
write.csv(x, here(paste0("Results/RF-pred-", response, "-occurrence-2.csv")), row.names=F)
          
x <- as.data.frame(ranger_obj$variable.importance)
write.csv(x, here(paste0("Results/RF-vimp-", response, "-occurrence-2.csv")), row.names=F)
...
write.csv(pd,  here(paste0("Results/RF-partialdependence-", response, "-occurrence-2.csv")), row.names=F)
```
...
MPBpred <- read.csv(here("Results/RF-pred-agentMPB-occurrence.csv"))
SBpred <- read.csv(here("Results/RF-pred-agentSB-occurrence.csv"))
WBBBpred <- read.csv(here("Results/RF-pred-agentWBBB-occurrence.csv"))
...
BBpred$suitablehosts <- factor(BBpred$agentsuitable, levels=c("none", "MPB", "SB", "WBBB", "MPB & SB",  "MPB & WBBB", "SB & WBBB",  "MPB, SB & WBBB"), labels=c("none", "PICO", "PIEN", "ABLA","PICO & PIEN",  "PICO & ABLA", "PIEN & ABLA",  "PICO, PIEN, & ABLA"), ordered=T)

```

## Host co-occurrence

### Given suitable stand conditions, is co-occurrence of suitable hosts common?
...
cocount <-  as.data.frame(table(bbo.dat$agentsFplongrev))
MPBco <-sum(cocount[cocount$Var1 %in% c("MPB & SB",  "MPB & WBBB","MPB, SB & WBBB"),]$Freq)
SBco <-sum(cocount[cocount$Var1 %in% c("MPB & SB",  "SB & WBBB","MPB, SB & WBBB"),]$Freq)
WBBBco <-sum(cocount[cocount$Var1 %in% c("MPB & WBBB",  "SB & WBBB","MPB, SB & WBBB"),]$Freq)

cocount <-  as.data.frame(table(bbo.dat$suitablehosts))
PICOco <-sum(cocount[cocount$Var1 %in% c("PICO & PIEN",  "PICO & ABLA","PICO, PIEN & ABLA"),]$Freq)
PIENco <-sum(cocount[cocount$Var1 %in% c("PICO & PIEN",  "PIEN & ABLA","PICO, PIEN & ABLA"),]$Freq)
ABLAco <-sum(cocount[cocount$Var1 %in% c("PICO & ABLA",  "PIEN & ABLA","PICO, PIEN & ABLA"),]$Freq)

codat <- data.frame(agent =c( "MPB", "SB", "WBBB"), agentco=c(MPBco, SBco, WBBBco), hostco=c(PICOco, PIENco, ABLAco))
...
ggfreq <- ggplot(bbo.dat[bbo.dat$focalhost=="Any",], aes(x=hostsFPshort,fill=agentsFplongrev))+geom_bar()+theme(legend.title = element_blank())+xlab("hosts")+scale_fill_manual(values=p2col)

ggsave( here("Results/Figures/FigureAgentfrequencybyhosts.jpg"), plot=ggfreq,   width=fig.width2,height=fig.width1.5,units="in")

```

### Is occurrence more likely or severity greater in stands with multiple hosts?
...
ggsave( here("Results/Figures/FigureOccurrencebySuitablehosts.jpg"), plot=ggoccurrence,  width=fig.width2, height=fig.width1,units="in")
...
ggseverity <- ggplot(dfpred, aes(x=focalagent, y=propTBA*100, fill=focalagent, colour=focalagent))+geom_boxplot(notch=T)+facet_grid(.~suitablehosts, scales="free_x", space="free_x")+xlab("bark beetle species")+scale_fill_manual(values=adjust_transparency(c(p1col[1:3], gray(0.3)), 0.5))+scale_colour_manual(values=c(p1col[1:3], gray(0.3)))+theme(legend.position = "none")+ylim(1,100)+geom_text(aes(label=pplot), vjust=-0.25,size=10, col="black")+ylab("Severity (% BA affected)")

ggsave(here("Results/Figures/FigureSevertitybySuitablehosts.jpg"), plot=ggseverity, width=fig.width2,height=fig.width1,units="in")
...
### What are the effects of multiple agents?
```{r severity by suitable hosts}
dfpred <- merge(BBpred, df, id.vars="PLT_CN", all=T)
dfpred <- dfpred[dfpred$suitablehosts %in% c("PICO & PIEN",  "PICO & ABLA", "PIEN & ABLA",  "PICO, PIEN, & ABLA"), ]
...
dfpred <- dfpred[!(dfpred$suitablehosts=="PICO & PIEN" & dfpred$agentsF %in% c("WBBB", "MPB &\nWBBB", "SB &\nWBBB", "MPB,\nSB, &\nWBBB")),]

dfpred <- dfpred[!(dfpred$suitablehosts=="PICO & ABLA" & dfpred$agentsF %in% c("SB", "MPB\n& SB", "SB &\nWBBB", "MPB,\nSB, &\nWBBB")),]

dfpred <- dfpred[!(dfpred$suitablehosts=="PIEN & ABLA" & dfpred$agentsF %in% c("MPB", "MPB\n& SB", "MPB &\nWBBB", "MPB,\nSB, &\nWBBB")),]


ggmultipelseverity <- ggplot(dfpred[dfpred$focalagent=="Any",], aes(x=agentsF, y=propTBA*100, fill=agentsF))+geom_boxplot()+facet_grid(.~suitablehosts, scales="free_x", space = "free_x")+theme(legend.position="none")

ggsave(here("Results/Figures/FigureSeverityMultiple.jpg"), plot=ggmultipelseverity , width=fig.width2,height=fig.width1,units="in")
```
...
```{r FigCoOccurrence, fig.cap = "Co-occurrence",  out.width = "504.8pt"}

knitr::include_graphics( here("Results/Figures/FigureAgentfrequencybyhosts.jpg"))
```
...
MPBpp <- read.csv(here("Results/RF-partialdependence-agentMPB-occurrence.csv"))
MPBpp$focalagent <- "MPB"
SBpp <- read.csv(here("Results/RF-partialdependence-agentSB-occurrence.csv"))
SBpp$focalagent <- "SB"
WBBBpp <- read.csv(here("Results/RF-partialdependence-agentWBBB-occurrence.csv"))
...
ppropBA1 <- ggplot(ppdat[ppdat$var=="propBA",], aes(x=value, y=yhat, col=focalagent))+geom_smooth()+scale_color_manual(values=p1col)+theme(legend.position="none")+ylab("probability of occurrence")+theme(axis.title.x=element_blank(),axis.text.x=element_blank() )

ppropBA2 <- ggplot(xm[xm$var=="propBA",],aes(x=value, y=focalagent, fill=interaction(focalagent, affected01)))+geom_boxplot()+scale_fill_manual(values=c(adjust_transparency(p1col[1:3], alpha=0.25), p1col[1:3]))+theme(legend.position="none")+ylab("")+xlab("propBA")
...
```{r FigStandSC,  fig.cap = "A caption", out.width = "322.56pt", eval=F}
...
jpeg(Fig.file, width=fig.width1.5, height=fig.width2/2, units="in")
